...
export default function ProductCategoriesTable({totalCategories}) {
  ...
  const columns = [
    {
      name: 'Id',
      maxWidth: '40px',
      cell: row => <p className="text-blueGray-700 uppercase"> {row.id}</p>,
      selector:  row => row.id,
      sortable: false,
    },
    {
    ...
    },
    {
      ...
      onClick={() => deleteCat(row.id)}>Delete
        </button>
    },
  ];
  ...
  const deleteCat = async id => {
    ...
    if (window.confirm(`Are you sure you want to delete selected category?`)) {
      ...
      const res = await fetch(`${process.env.API_URL}/product_categories/${id}`, {
        method: 'DELETE',
      });
      ...
      getCategories(1,"")
      setAlertType('info');
      setAlertContent('Category Deleted');
      setOpen(true);

    }
  }

  const deleteCatMany = async ids => {
    const res = await fetch(`${process.env.API_URL}/product_categories/deletemany`, {
      method: 'DELETE',
      headers: {
    ...
    getCategories(1,"");
    setAlertType('info');
    setAlertContent('Categories Deleted');
    setOpen(true);
  }
  ...
  const AddCategory = async event => {
   ...
   if(name == "") {
    setAlertType('error');
    setAlertContent('Add Category Name');
    setOpen(true);
   ...
   if(slug == "") {
    setAlertType('error');
    setAlertContent('Add Category Slug');
    setOpen(true);

      return
   }

    const res = await fetch(`${process.env.API_URL}/product_categories`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    setAlertContent('Category Created');
    setModal(false);
...
const EditCategory = async event => {
 ...
 if(slug == "") {
  ...
  getCategories(1,"");
  setAlertType('success');
  setAlertContent('Category Updated');
  setEditModal(false);
...
///////////////////////////
  ...
  async function getCategories(page = 1, text = '') {

      const res = await fetch(`${process.env.API_URL}/product_categories?page=${page}&per_page=${countPerPage}&text=${text}`);
      const allcategories = await res.json();

      setTotal(allcategories.total);
      ...
      <div className="flex flex-wrap py-3">
        <div className="w-full px-4">
          ...
          {!pending && <div className="relative flex w-full  mb-3 items-center flex justify-between">
              ...
              <input onChange={(e) => filterCategories(e.target.value)} type="text" placeholder="Search Categories" className="px-3 py-3 placeholder-blueGray-300 text-blueGray-600 relative bg-white bg-white rounded text-sm shadow outline-none focus:outline-none focus:shadow-outline lg:w-4/12 pl-10"/>
              <button className="bg-green-500 active:bg-green-500 text-white font-bold 
 uppercase text-xs px-4 py-3 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-150" onClick={() => setModal(!modal)}>Add New Category</button>  
      ...
      <Dialog open={modal} className="px-4 py-4">
        <DialogTitle className="text-center mb-6">Create New Product Category</DialogTitle>
        <DialogContent>
          <form onSubmit={AddCategory}>
            <div className="flex flex-wrap items-center justify-center"> 
              ...
              <div className="w-full lg:w-8/12 xl:w-12/12 px-4 mb-6">
                <select className="px-3 py-3 placeholder-blueGray-300 text-blueGray-600 relative bg-white bg-white rounded text-sm shadow outline-none focus:outline-none focus:shadow-outline w-full pl-10" id="parent" label="Parent Category" name="parent">
 ...
 uppercase text-xs px-4 py-3 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-150" onClick={handleModalClose}>Cancel</button>
      ...
      <Dialog open={editmodal} className="px-4 py-4">
        <DialogTitle className="text-center mb-6"> Edit Product Category</DialogTitle>
        <DialogContent>
          <form onSubmit={EditCategory}>
            <div className="flex flex-wrap items-center justify-center"> 
              ...
              <div className="w-full lg:w-8/12 xl:w-12/12 px-4 mb-6">
                <select onChange={handleInputChange} value={formvalues.parent}  className="px-3 py-3 placeholder-blueGray-300 text-blueGray-600 relative bg-white bg-white rounded text-sm shadow outline-none focus:outline-none focus:shadow-outline w-full pl-10" id="parent" label="Parent Category" name="parent">
                        <option value={0}>Select Parent Category</option>
                        {categories.map(cat => {
                      return (
                        <option key={cat.id} value={cat.id}>{cat.name}</option>
