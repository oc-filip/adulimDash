...
export default function PersonsTable({totalPersons}) {
...
const columns = [
  {
    name: 'User ID',
    cell: row => <Link href={`persons/${row.id}`}>
  ...
  {
    name: 'Rihvit ID',
    selector: row => row.b38f26908a473cb2993a861d9590b687a9a0ca15,
    sortable: false,
    cell: row => <div className="flex flex-col"><p className="text-blueGray-500">{row.b38f26908a473cb2993a861d9590b687a9a0ca15} </p></div>,
  },
  {
    right: true,
    maxWidth: '120px',
  ...
  async function getCustomers(page = 1,  text= '') {
      ...
      const res = await fetch(`${process.env.API_URL}/persons?page=${page}&per_page=${countPerPage}&text=${text}`);
  ...
  const deleteUser = async id => {
    ...
    if (window.confirm(`Are you sure you want to delete selected user?`)) {
      ...
      const res = await fetch(`${process.env.API_URL}/persons/${id}`, {
        method: 'DELETE',
      });
  ...
  const deleteUserMany = async ids => {
    const res = await fetch(`${process.env.API_URL}/persons/deletemany`, {
      method: 'DELETE',
      headers: {
...
//////////////////////////////////////////////////////
  ...
  async function getTotal(role) {
    ...
    if (role === 'any') {
      ...
      const res = await fetch(`${process.env.API_URL}/persons/gettotals?role=${role}`);
      const totalcustomers = await res.json();
      setTotal(totalcustomers.total);
        ...
        <div className="w-full px-4">
          <div className="block w-full overflow-x-auto">
            ...
            {!pending && <div className="relative flex w-full  mb-3 items-center flex justify-between">
                    ...
                    <input onChange={(e) => filterUsers(e.target.value)} type="text" placeholder="Search Persons" className="px-3 py-3 placeholder-blueGray-300 text-blueGray-600 relative bg-white bg-white rounded text-sm shadow outline-none focus:outline-none focus:shadow-outline lg:w-4/12 pl-10"/>
                    <button className="bg-green-500 active:bg-green-500 text-white font-bold 
      uppercase text-xs px-4 py-3 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-150" onClick={() => setModal(!modal)}>New Person</button>  
